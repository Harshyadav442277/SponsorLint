...
**Status:** FINAL — sole execution authority
...
# 0. THIS FILE IS THE AUTHORITY

This document is the final build contract. Any coding agent, teammate, or assistant treats it as the **single source of truth**.

It supersedes and replaces:

- `SPONSORLINT_FINAL_EXECUTION_BIBLE.md`
- `SPONSORLINT_EXECUTION_BIBLE.md`
- `SponsorLint_Project_Bible.md`
- `BAKEOFF.md` and `STRATEGY.md` (decision records, not implementation authorities)
- All Cutcheck documents

**Do not keep two bibles in the repo.** Archive the others. Conflicting authorities are exactly the failure mode every one of them warns about.

If this file conflicts with an older plan, an agent suggestion, "this would look cooler," or "a production system should," **this file wins**.
...
Always display the disclosure timestamp.

**If the brief specifies placement** (as the demo brief does — *"near the beginning"*), enforce it as a normal rule.

**If the brief does not specify placement** and disclosure is unusually late, emit an advisory only:

```text
⚠ ADVISORY
Disclosure detected at 00:47. Review placement before sending.
```

**Do not call it legal non-compliance. Do not quote regulatory language.** You check the supplied brief, not the law (§24).
